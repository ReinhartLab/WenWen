...
# define masks
# if not exist, create FFA mask from the Harvard-Oxford Cortical Structural Atlas

from nilearn import datasets, image

roi_thresh = 50 # probability of being in the label >=??
roi_res = '2mm'
harvard_oxford_fusiform = join(myDir['templates'],f'fusiform_mask_havard_oxford_thr{roi_thresh}-{roi_res}.nii.gz')

if not os.path.exists(harvard_oxford_fusiform):

    harvard_oxford = datasets.fetch_atlas_harvard_oxford(f'cort-maxprob-thr{roi_thresh}-{roi_res}')
    fusiform_labels = [label for label in harvard_oxford.labels if 'fusiform' in label.lower()]
    # print(fusiform_labels)
    # Extract the fusiform region from the atlas
    fusiform_prob_map = harvard_oxford.maps
    fusiform_prob_data = harvard_oxford.maps.get_fdata()

    # Get the indices of the fusiform labels in the Harvard-Oxford atlas
    fusiform_indices = [harvard_oxford.labels.index(label) + 1 for label in fusiform_labels]

    # Threshold the probability map to create a binary mask of the fusiform region
    fusiform_mask_data = np.isin(fusiform_prob_data, fusiform_indices).astype(np.int16)  # Use int16 to avoid int64 issue

    # Create a new nibabel image from the fusiform mask data
    fusiform_mask_img = nib.Nifti1Image(fusiform_mask_data, fusiform_prob_map.affine)

    nib.save(fusiform_mask_img, harvard_oxford_fusiform)
    # plotting.plot_roi(fusiform_mask_img, title="Fusiform Mask", cmap='Paired', colorbar=True)

ffa_hvd_ox = load_img(harvard_oxford_fusiform)

# using template from Nancy Kanwisher's GSS CVS registration
# https://web.mit.edu/bcs/nklab/GSS.shtml

img_name = join(myDir['templates'],'cvs_face_parcels', 'fROIs-fwhm_5-0.0001.nii')
ffa_gss_atlas = load_img(img_name)

# Squeeze the 4D image to 3D
ffa_gss_atlas = image.index_img(ffa_gss_atlas, 0)
...
def plot_glm_and_mask(thresh_p, cluster_vox_num, hc,isow,ffa_gss_atlas,ffa_hvd_ox):  
    ...
    ffa_gss_atlas = image.resample_to_img(ffa_gss_atlas, glm_result)
    ffa_gss_atlas = image.math_img('img > 1.96', img=ffa_gss_atlas) # mask p<0.05
    
    # resample and binarize, using 0.01 at the cutoff to avoid spurious ROI resulted from value intepolation
    ffa_hvd_ox = image.resample_to_img(ffa_hvd_ox, glm_result)
    ffa_hvd_ox = image.math_img('img > 0.01', img=ffa_hvd_ox)
    
    # combined_mask_img = ffa_hvd_ox
    # combined_mask_img = ffa_gss_atlas
    combined_mask_img = image.math_img('(mask1_img > 0) | (mask2_img > 0)', mask1_img=ffa_gss_atlas, mask2_img=ffa_hvd_ox)

    # Apply mask to the GLM result
    masked_glm_result = image.math_img('glm_result * roi_mask', glm_result=glm_result, roi_mask=combined_mask_img)
    masked_glm_result.to_filename(res_filename)

    fig, axes = plt.subplots(3, 1, figsize=(5, 10))

    plotting.plot_roi(combined_mask_img, title='FFA Mask', axes=axes[0])
...
for thresh_p in thresh_p_values:
    for cluster_vox_num in cluster_vox_num_values:
        for hc in hc_values:
            ...
            plot_glm_and_mask(thresh_p, cluster_vox_num, hc,isow,ffa_gss_atlas,ffa_hvd_ox)
