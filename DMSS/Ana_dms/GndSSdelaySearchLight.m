    ...
    for cond = 1:2
        ...
        for g = 1:2
            for t = 1:length(toi)-1
                ...
                title(sprintf('%s:%.1f~%.1fs', groupStr{g},toi(:,t),toi(:,t+1)),sprintf('%s, p<%.3f',condStr{cond},threshP));
            end
            ...
            dat_mn = squeeze(mean(fem.sl.mn{g}(cond,:,:),3));
            dat_all = squeeze(mean(fem.sl.data{g}(:,cond,:,:),4));
            [~,p] = ttest(dat_all,1/3,'Tail','right');
            ...
            title(sprintf('%s:0~3s', groupStr{g}),sprintf('%s, p<%.3f',condStr{cond},threshP));
        ...
        h = colorbar;
        set(h,'ytick',[-0.02 0.02])
        title(sprintf('%s:%s avg', groupStr{g},condStr{cond}),sprintf('p<%.2f',threshPdiff));
