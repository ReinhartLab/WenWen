function single_variablity_response_ft_oddeven(sn,IsLap,IsdePhase,IsCorretTrials,IsBL2preDelay,IsOverwrite)
% IsBL2preDelay: use pre-trial interval or pre response as baseline; better named as IsBL2preResp
load('subs.mat');
subname = subs.name{sn};

txtCell = {'','','','';'_lap','_dephase','_corrTrials','_bl2preDelay'};
outputFile =fullfile(Dir.results,[subname,'_resp_ft_var_oddeven',txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.mat']);
...
if isfile(set_name)
    ...
    for cond_i = 1:3
        ...
        for i = 1:2
            ...
            tmpID = mod(i,2)+(1:2:sEEG.trials-2);% for nv
            tmpTrl = condTrials(tmpID);% for beha

            PostTrlBeha{cond_i,i} = M(tmpTrl+1,:);
            ...
            cfg = [];
            cfg.latency = [-1 inf];
            tfDat{cond_i,i} = ft_selectdata(cfg,tmp_eeg);
        end
    end
    %%
