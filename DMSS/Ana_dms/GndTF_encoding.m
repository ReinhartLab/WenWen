...
writetable(T,fullfile(Dir.ana,'TableOutput',['EncodingBetaPower_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.csv']))
...
% writetable(T,fullfile(Dir.ana,'TableOutput',['EncodingBetaPower_Slope_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.csv']))
...
for gi = 1:2
    ...
    set(gca,'XTick',[-4.8 -3.6 -2.4 -1.2 0],'YLim',[-1.2 0.5],'XLim',[-5.23 timeROI.all(2)])
...
set(gca,'YLim', [-0.6 0.2],'ytick',-0.6:0.2:0.6,'XLim',[0 3]);
ylabel({'\bfSlope'});
...
text(1.5,-0.45,['p=',num2str(p)],'HorizontalAlignment','center','FontSize',12);
plot([1 2],[-0.5 -0.5],'k')

saveas(gca,fullfile(Dir.figs,['EncodingBetaPower_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.png']))
    ...
    for idx = 1:2
        ...
        for gi = 1:2
            for cond = 1:3
                ...
                g(gi,cond).set_names('x',['BetaPower of ' var_Str{var_idx}],'y',behaStr{idx}); %Set the title of the axis
        ...
        g.draw();
        saveas(gcf,fullfile(Dir.figs,['EncodingPowCorr_',behaStr{idx},var_Str{var_idx},num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.png']))
    end
end