...
IsCorretTrials = 1;
IsBL2preDelay = 1;% baseline to pre-resp
IsLap = 1;
IsdePhase=1;
...
timeROI.delay = [0.1 0.5];% in s

%%
clear subsAll
for sub_i = 1:subN
    subname = subs.name{sub_i};
    matName = fullfile(Dir.results,[subname,'_SNR_resp_',txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.mat']);
...
saveas(gcf,fullfile(Dir.figs,['SNR_resp_',[chanROI{:}],num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz_',txtCell{IsLap+1,1},num2str(timeROI.delay(1)),'~',num2str(timeROI.delay(2)),'s',txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.png']))
