function single_delay_VariabilityGuidedPower_nn_Nsampled(sn,Nsamps,IsLap,IsdePhase,IsCorretTrials,IsBL2preDelay,IsOverwrite)
...
% get the trialwise power data and behavior.

load('subs.mat');
subname = subs.name{sn};

txtCell = {'','','','';'_lap','_dephase','_corrTrials','_bl2preDelay'};
outputFile =fullfile(Dir.results,[subname,'_delay_varGpow_seq_NsampledAT',num2str(Nsamps),txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.mat']);
...
if isfile(set_name)
    ...
    for b = 1:sEEG.trials-Nsamps-1% moving the trial cluster one by one
        ...
        avgBL = mean(mean(bl.powspctrm(tmpID,:,:,:),1),timedimB);%  trial (1)*chan*freq
    ...
    %%
    save(outputFile,'tfDat','TrlBeha','PowTrl','-v7.3')
end
