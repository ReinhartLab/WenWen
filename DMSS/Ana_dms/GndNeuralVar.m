...
saveas(gcf,fullfile(Dir.figs,['NeuVar_',[frontalROI{:}],txtCell{IsLap+1,1},num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.png']))
...
saveas(gcf,fullfile(Dir.figs,['NeuVarGroupDiff_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.png']))
...
writetable(T,fullfile(Dir.ana,'TableOutput',['DelayBetaVariance_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.delay(1)),'~',num2str(timeROI.delay(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.csv']))
...
writetable(T,fullfile(Dir.ana,'TableOutput',['DelayBetaVariance_Slope_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.delay(1)),'~',num2str(timeROI.delay(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.csv']))
...
for gi = 1:2
    subplot(3,4,(gi-1)*4+[1 2]);hold all;
    for cond_i = 1:3
    ...
    set(gca,'XTick',[-4.8 -3.6 -2.4 -1.2 0 3 timeROI.all(2)],'YLim',[-1.1 1],'XLim',[-5.23 4],'YTick',-1:0.5:1)
%     set(gca,'XTick',[-4.8 -3.6 -2.4 -1.2 0 3 4],'YLim',[-1.2 1.2],'XLim',[-1.4 4])
    ...
    plot([3 3],get(gca,'YLim'),'k','LineWidth',0.5,'Color',[1 1 1]*0.9,'HandleVisibility','off')
...
subplot(3,4,10);hold all;axis square
...
subplot(3,4,11);hold all;axis square
...
ytickformat('%.1f')
ylabel('Variance(AU)')
title(sprintf('%s\nbeta preDelay (%.1f~%.1fs)',[frontalROI{:}],timeROI.preDelay(1),timeROI.preDelay(2)))
if Ps.betaFrontal{3}<.05
    for gi = 1:2
...
subplot(3,4,12);hold all;axis square
...
saveas(gca,fullfile(Dir.figs,['DelayBetaVariance_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.png']))

fig.PaperOrientation = 'landscape';
print(fig,fullfile(Dir.figs,['DelayBetaVariance_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.pdf']),'-dpdf','-r300','-bestfit')
%% plot slope index 

fig=figure('Position',[200 200 650 300]);
...
plot(xcord(1,:),dat.betaAvgOccip{1},'Color',[0.8 0.8 0.8],'LineWidth',0.5,'HandleVisibility','off');
plot(xcord(2,:),dat.betaAvgOccip{2},'Color',[0.8 0.8 0.8],'LineWidth',0.5,'HandleVisibility','off');
hb = bar(mn,'FaceAlpha',0.8);
hb(1).FaceColor = myColors(1,:);
hb(2).FaceColor = myColors(2,:);
hb(3).FaceColor = myColors(3,:);
xcord = vertcat(hb(:).XEndPoints)';
errorbar(xcord',mn',se','k','LineWidth',1.5)
legend(condStr,'Location','bestoutside')
set(gca,'xtick',[1 2],'XTickLabel',groupStr,'XLim',[0.5 2.5],'YLim',[-1.2 0],'YTick',[-1.2:0.6:1.6],'YDir','reverse')

ylabel('Variance(AU)')
title(sprintf('%s\nbeta delay (%.1f~%.1fs)',[frontalROI{:}],timeROI.delay(1),timeROI.delay(2)))


subplot(3,4,11);hold all;box on;
% subs.slope = [line_fit{2}.params(:,1);line_fit{1}.params(:,1)];
% boxplot(subs.slope,subs.groupStr)
% plot(get(gca,'xlim'),[0 0],'k--')
% set(gca,'YLim', [-0.4 0.4],'ytick',-0.4:0.2:0.6);
% ylabel({'\bfSlope'});

bar([1 2],[mean(line_fit{1}.params(:,1)) mean(line_fit{2}.params(:,1))],'w','BarWidth',0.5)
...
set(gca,'YLim', [-0.1 0],'ytick',-0.4:0.1:0.6,'YDir','reverse','XTick',[1 2],'XTickLabel',groupStr,'XLim',get(gca,'xlim')+[0.15 -0.15]);
ylabel({'\bfSlope'});
for gi = 1:2
    if  line_fit{gi}.p<.05
        text(gi,-0.08,'*','HorizontalAlignment','center','FontSize',20)
    end
end
saveas(gcf,fullfile(Dir.figs,['DelayBetaVarianceSlope_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.png']))
print(fig,fullfile(Dir.figs,['DelayBetaVarianceSlope_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.pdf']),'-dpdf','-r300','-bestfit')
...
saveas(gca,fullfile(Dir.figs,['DelayAlphaVariance_',num2str(freq.alphaFreq(1)),'~',num2str(freq.alphaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.png']))
...
for idx = 2%1:2
    ...
    for gi = 1:2
        ...
        mdl_24 =  stepwiseglm(X,Y,'linear','Distribution','normal','Criterion','bic','Lower','linear');
        ...
        g(gi,cond).set_text_options('base_size' ,10,'title_scaling' ,1.1);%Set the font size, the base font size base_size is set16, the title font size of the axis is set to the base font size1.2Times
        if idx == 2
            g(gi,cond).axe_property('XLim',[-1 0.3],'YLim',[0.6 1],'YTick',[0.6:0.1:1],'XTick',[-0.8:0.2:0.2]);
        else
            g(gi,cond).axe_property('XLim',[-1 0.3],'YLim',[0.4 1.4],'YTick',[0.4:0.2:2],'XTick',[-0.8:0.2:0.2]);
        end
%         g(gi,cond).set_color_options('map','brewer1');
    ...
    g.draw();
    saveas(gcf,fullfile(Dir.figs,['NeuVarCorr_',behaStr{idx},num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.png']))
    fig.PaperOrientation = 'landscape';
    print(fig,fullfile(Dir.figs,['NeuVarCorr_',behaStr{idx},num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.pdf']),'-dpdf','-r300','-bestfit')
end
...
saveas(gcf,fullfile(Dir.figs,['DelayVarCorrSlope_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI.all(1)),'~',num2str(timeROI.all(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.png']))
...
figure('Name','Load-sensitive chans','Position',[100 200 2000 600]);
