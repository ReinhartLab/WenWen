...
for gi = 1:3
    ...
    xlabel('\bfTime(s)')
    rectangle('Position',[0 -0.5 3 50],'LineWidth',1.2,'LineStyle','--')
...
for gi = 1:2
    subplot(3,2,gi);hold all;axis square;
    for cond_i = 1:3
    ...
    subplot(3,2,gi+2);hold all;axis square;
    for cond_i = 1:3
...
g.draw();
saveas(gcf,fullfile(Dir.figs,['NeuVarCorr_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI(1)),'~',num2str(timeROI(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.bmp']))
