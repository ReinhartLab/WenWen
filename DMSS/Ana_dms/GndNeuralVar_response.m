...
IsCorretTrials = 1;
IsBL2preDelay = 1;% means pre response
IsLap = 1;
IsdePhase=1;
...
timeROI = [-0.4 0.5];% in s, for curve plotting
timeROIanovaPost = [0 0.5];% in s
timeROIanovaPre = [-0.3 0];% in s
...
for gi = 1:2
    ...
    for cond_i = 1:3
        ...
        rectangle('Position',[0 0.5 0.6 40],'LineWidth',1.2,'LineStyle','--')
    ...
    saveas(gcf,fullfile(Dir.figs,['NeuVar_Resp_',groupStr{gi},[frontalROI{:}],txtCell{IsLap+1,1},num2str(timeROI(1)),'~',num2str(timeROI(2)),'s',txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.bmp']))
end
...
for gi = 1:3
    ...
    rectangle('Position',[0 -1 0.6 50],'LineWidth',1.2,'LineStyle','--')
    set(gca,'ytick',0:10:length(gndTF{1}.freq),'XTick',timeROI,'XLim',timeROI,'YLim',[0.5 40])

end
saveas(gcf,fullfile(Dir.figs,['NeuVar_Resp_GroupAvg_',[cfg.channel{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.bmp']))
%% topography
...
timeIDanovaPost = dsearchn(gndTF{1}.time',timeROIanovaPost');
timeIDanovaPost = timeIDanovaPost(1):timeIDanovaPost(2);

timeIDanovaPre = dsearchn(gndTF{1}.time',timeROIanovaPre');
timeIDanovaPre = timeIDanovaPre(1):timeIDanovaPre(2);
...
for gi = 1:2
    for cond_i = 1:3
        dat.betaAvgFrontal{gi}(:,cond_i) = squeeze(mean(mean(mean(gndTF{gi,cond_i}.indv(:,chanID.frontal(log1),freqID.beta,timeIDanovaPost),2,"omitnan"),3,"omitnan"),4,"omitnan"));
        dat.betaAvgOccip{gi}(:,cond_i) = squeeze(mean(mean(mean(gndTF{gi,cond_i}.indv(:,chanID.occip(log2),freqID.beta,timeIDanovaPost),2,"omitnan"),3,"omitnan"),4,"omitnan"));
        dat.betaAvgFrontalPre{gi}(:,cond_i) = squeeze(mean(mean(mean(gndTF{gi,cond_i}.indv(:,chanID.frontal(log1),freqID.beta,timeIDanovaPre),2,"omitnan"),3,"omitnan"),4,"omitnan"));
...
ytickformat('%.1f')
ylabel('Variance(a.u.)')
title(sprintf('%s beta\nPost(%.1f~%.1fs)',[frontalROI{:}],timeROIanovaPost(1),timeROIanovaPost(2)))
% plot significance
...
ytickformat('%.1f')
ylabel('Variance(a.u.)')
title(sprintf('%s beta(%.1f~%.1fs)',[occipROI{:}],timeROIanovaPost(1),timeROIanovaPost(2)))
...
ytickformat('%.1f')
ylabel('Variance(a.u.)')
title(sprintf('%s beta\nPre(%.1f~%.1fs)',[frontalROI{:}],timeROIanovaPre(1),timeROIanovaPre(2)))
...
saveas(gca,fullfile(Dir.figs,['RespBetaVariance_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI(1)),'~',num2str(timeROI(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.bmp']))
%% correlation
load beha.mat
...
for gi = 1:2
    ...
    for cond = 1:3
        ...
        g(gi,cond).set_names('x','Beta variance','y','Beha'); %Set the title of the axis
...
for gi = 1:2
    ...
    g(gi,cond).geom_point(); %Draw a scatter plot
    g(gi,cond).stat_glm(); %Draw lines and confidence intervals based on scatter plots
    g(gi,cond).set_names('x',sprintf('Beta variance(%d~%dHz)',freq.betaFreq(1),freq.betaFreq(2)),'y','Beha'); %Set the title of the axis
    g(gi,cond).set_color_options('map',myColors(cond,:)); %Set the color of the point
...
g.draw();
saveas(gcf,fullfile(Dir.figs,['RespNeuVarCorr_',num2str(freq.betaFreq(1)),'~',num2str(freq.betaFreq(2)),'Hz',num2str(timeROI(1)),'~',num2str(timeROI(2)),'s',[frontalROI{:}],txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4} '.bmp']))
