function single_power_response_ft_Nplus1Correlation_Nsampled(sn,Nsamps,IsLap,IsdePhase,IsCorretTrials,IsBL2preDelay,IsOverwrite)
% IsBL2preDelay: use pre-trial interval or pre response as baseline; better named as IsBL2preResp
load('subs.mat');
subname = subs.name{sn};

txtCell = {'','','','';'_lap','_dephase','_corrTrials','_bl2preDelay'};
outputFile =fullfile(Dir.results,[subname,'_resp_ft_power_NsampledAT',num2str(Nsamps),txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsCorretTrials+1,3},txtCell{IsBL2preDelay+1,4},'.mat']);
...
if isfile(set_name)
    ...
    for cond_i = 1:3
        ...
        Nsamps = min([sEEG.trials-20,Nsamps]);% in case no enough trials to sample from, I set an arbitrary value of EEG.trials-20
        ...
        for b = 1:sEEG.trials-Nsamps-1% moving the trial cluster one by one but skip the last two to avoid error of N+1
            ...
            avg = squeeze(mean(eeg.powspctrm(tmpID,:,:,:),1,"omitnan"));%chan*freq*time, take the mean of a cluster of trials

            tmp_eeg = ft_freqdescriptives([],eeg);% get ft structure;
            tmp_eeg.powspctrm = avg;
