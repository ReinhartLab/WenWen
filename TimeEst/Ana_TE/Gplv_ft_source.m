...
ROI.roi = [11 12 13 14];%A10m,http://atlas.brainnetome.org/bnatlas.html
...
for sn = 1:height(subs)
    ...
    outFile = fullfile(Dir.results,[subname,'_PLVsource','0.1~0.5',txtCell{IsdePhase+1,2},'.mat']);
...
stat.mask(ROI.in) = 0;% mask out seed region
stat.stat(ROI.in) = 0;% mask out seed region
...
print(fullfile(Dir.figs,['source_int_2D',num2str(x.source_conn_full.toi(1)),'~',num2str(x.source_conn_full.toi(2)),txtCell{IsdePhase+1,2},'.png']),'-dpng','-r300');
...
ft_sourceplot(cfg, statint);
title(condStr{3});
print(fullfile(Dir.figs,['source_Slice',num2str(x.source_conn_full.toi(1)),'~',num2str(x.source_conn_full.toi(2)),txtCell{IsdePhase+1,2},'.png']),'-dpng','-r300');
...
for a = 1:length(viewAng)
    ...
    title(condStr{3});
    print(fullfile(Dir.figs,['source_int_3D',num2str(x.source_conn_full.toi(1)),'_',num2str(x.source_conn_full.toi(2)),'_',num2str(a),txtCell{IsdePhase+1,2},'.png']),'-dpng','-r300');
end
...
%     title(condStr{3});
% 
%     print(fullfile(Dir.figs,['source_int_inflated',num2str(x.source_conn_full.toi(1)),'_',num2str(x.source_conn_full.toi(2)),'_',num2str(a),txtCell{IsdePhase+1,2},'.png']),'-dpng','-r300');
% end
%%
