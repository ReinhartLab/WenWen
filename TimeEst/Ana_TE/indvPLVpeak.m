...
for sn = 1:height(subs)
    ...
    tmp = locs<=PeakPara.freq(2) & locs>=PeakPara.freq(1);
    Indv.peakF{sn,1} = locs(tmp);

    subplot(2,1,2);hold all;box on
    plot(tfDat{1}.plvFT.freq,freqCurve)
    plot(locs,pks,'ro')
    for li = 1:length(pks)
        text(locs(li),pks(li)+0.02,sprintf('%.1fHz',tfDat{1}.plvFT.freq(locs(li))),'HorizontalAlignment','center')
    end
...
saveas(gcf,fullfile(Dir.figs,'IndvPLVpeak',[sprintf('Boxplot%.1f~%.1fs%d~%dHz',PeakPara.time(1),PeakPara.time(2),PeakPara.freq(1),PeakPara.freq(2)),sprintf('%s (ref at %s)',[cfg.channel{:}],[cfg.refchannel{:}]),txtCell{IsLap+1,1},txtCell{IsdePhase+1,2},txtCell{IsBL+1,3},'.png']))
